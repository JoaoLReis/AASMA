...
public class Builder : MonoBehaviour, ReactiveInterface {
    ...
    {
        gameSpeed = hub.gameSpeed;
        if (currentBuildingMaterials < 0.1 * maxBuildingMaterials)
            goRefill = true;
        if (!detectIfRefill())
        {
            if (collided)
            ...
            if (preparingToBuild && freeBuildArea != null)
            {
                ...
                if (transform.rotation == rot && !building)
                {
                    if ((freeBuildArea.transform.position - transform.position).magnitude < 13)
                    {
                        ...
                        transform.position = Vector3.MoveTowards(transform.position, freeBuildArea.transform.position, 10f * Time.fixedDeltaTime * gameSpeed);
                }
            }
        }
    }
